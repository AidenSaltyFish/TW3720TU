       ...
       int              maxIter = 100)
{
    ...
    Vector<T> p;
    Vector<T> r_cur, r_next; 
    T alpha_k(0), beta_k(0);

    p = r_cur = b - A * x;
    // std::cout << "p/r_cur here:" << std::endl;
    // p.print();
    // r_cur.print();

    for (int k = 0; k < maxIter; ++k) {
        const T r_cur_innerProduct(dot(r_cur, r_cur));

        alpha_k = r_cur_innerProduct / dot((A * p), p);
        // std::cout << "A * p here:" << std::endl;
        // (A * p).print();
        // std::cout << "dot((A * p), p) = " << dot((A * p), p) << std::endl;
        ...
        r_next = r_cur -  alpha_k * (A * p); // put scalar at tail

        // std::cout << "r_next here:" << std::endl;
        // r_next.print();
        // std::cout << dot(r_next, r_next) << " and " << total_square << std::endl;

        const T r_next_innerProduct(dot(r_next, r_next));
...
public:
    Heat(const T& _alpha, int _point_per_dim, const T& _dt) 
    ...
    {
        // M = Matrix<T>(total_point, total_point); 
        ...
        for (unsigned int i = 0; i < total_point; ++i) {
            ...
            // u_0 *= sin(M_PI * (i % point_per_dim + 1) * dx);
            if (u_0 > 1e-10) { u_0_vec[i] = u_0; }
        }
    ...
    Vector<T> exact(T t) const {
        // const double u_coeff(exp(-n * pow(M_PI, 2) * alpha * t));
        // .Vector<T> resVec(total_point); // u

        /*
        // FIXME : loop unrolling maybe
        for (unsigned int i = 0; i < total_point; ++i) {
            resVec[i] = u_coeff * u_0_vec[i];
        }
        */
        
        /*
        resVec = u_0_vec * u_coeff;
        return resVec;
        */

        return (u_0_vec * (exp(-n * pow(M_PI, 2.0) * alpha * t)));
    }

    Vector<T> solve(T t) const {
        Vector<double> resVec(total_point);
		Vector<double> init(u_0_vec);

        const unsigned int iterNum((unsigned int)(t / dt));
		for(unsigned int i = 0; i < iterNum; ++i) {
            resVec.reset();

            //cg<double>(M, init, resVec, 0.1, 1000)
            
			// std::cout << "success: " << cg<double>(M, init, resVec, 1e-8, 1000) << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    Heat<3, double> heat(0.3125, 3, 0.1);
    auto r = heat.exact(1.0);
    std::cout << "exact result starts here." << std::endl;
    r.print();

    auto s = heat.solve(1.0);
